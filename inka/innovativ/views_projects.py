...
def p_02_2_ujabb_megkereses(request, project, task_id):
    task = Task.objects.get(pk=task_id)
    if task.completed_at:
        messages.success(request, f'Ez a projekt már elkészült '
                                  f'{task.completed_at.strftime("%Y.%m.%d. %H:%M")}-kor.')
...
def p_04_1_elozetes_arajanlat_adas(request, project, task_id):
    ...
    if task.completed_at:
        ...
        return render(request, '04/p_04_1_elozetes_arajanlat_adas.html',
                      {'project': project, 'task': task, 'tasks': tasks_page,
                       'page_list': tasks_page, 'page_range': page_range,
                       'type_choices': type_choices, 'type_color': type_color,})
...
def p_05_1_felmeres(request, project, task_id):
    ...
    if task.completed_at:
        ...
        return render(request, '05/p_05_1_felmeres.html',
                      {'project': project, 'task': task, 'tasks': tasks_page,
                       'page_list': tasks_page, 'page_range': page_range,
                       'type_choices': type_choices, 'type_color': type_color, })


def p_05_2_ugyfel_egyeztetes_felmeresrol(request, project, task_id):
    task = Task.objects.get(pk=task_id)
    if task.completed_at:
        messages.success(request, f'Ez a projekt már elkészült '
                                  f'{task.completed_at.strftime("%Y.%m.%d. %H:%M")}-kor.')
...
def p_05_4_felmeresi_anyag_feldolgozasa(request, project, task_id):
    task = Task.objects.get(pk=task_id)
    if task.completed_at:
        messages.success(request, f'Ez a projekt már elkészült '
                                  f'{task.completed_at.strftime("%Y.%m.%d. %H:%M")}-kor.')
        ...
        return render(request, '05/p_05_4_felmeresi_anyag_feldolgozasa.html',
                      {'project': project, 'task': task, 'tasks': tasks_page,
                       'page_list': tasks_page, 'page_range': page_range,
                       'type_choices': type_choices, 'type_color': type_color,})